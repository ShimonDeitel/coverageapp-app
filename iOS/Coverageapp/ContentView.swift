...
struct ContentView: View {
    ...
    @State private var editingEntry: CoverageappEntry?
                        ...
                        ForEach(store.entries) { entry in
                            Button {
                                ...
                                VStack(alignment: .leading, spacing: 4) {
                                    ...
                                    if !entry.employeeName.isEmpty {
                                        Text("Employee: \(entry.employeeName)")
                                            .font(.caption)
                                            .foregroundStyle(Theme.accent)
                                    }
                                }
...
enum EntryFormMode {
    case add
    case edit(CoverageappEntry)
}

struct EntryFormView: View {
    let mode: EntryFormMode
    var onSave: (CoverageappEntry) -> Void
        ...
        NavigationStack {
            Form {
                Section {
                    ...
                    TextField("Employee", text: $field2)
                        .accessibilityIdentifier("field2Field")
                        .focused($focusedField, equals: .field2)
                    TextField("Covered By", text: $field3)
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        var entry: CoverageappEntry
                        if case .edit(let existing) = mode {
                            entry = existing
                        } else {
                            entry = CoverageappEntry(title: "")
                        }
                        entry.title = title.isEmpty ? "Untitled" : title
                        entry.date = date
                        entry.employeeName = field2
                        entry.coveredBy = field3
                        entry.note = note
                        onSave(entry)
                        dismiss()
                    }
            ...
            .onAppear {
                if case .edit(let existing) = mode {
                    ...
                    date = existing.date
                    field2 = existing.employeeName
                    field3 = existing.coveredBy
                    note = existing.note
                }
            }
        }
