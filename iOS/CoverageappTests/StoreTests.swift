...
final class StoreTests: XCTestCase {
    ...
    func testAddIncreasesCount() {
        ...
        let ok = store.add(CoverageappEntry(title: "Test"))
        XCTAssertTrue(ok)
    ...
    func testAddFailsAtFreeLimit() {
        ...
        while store.entries.count < Store.freeLimit {
            _ = store.add(CoverageappEntry(title: "Filler"))
        }
        let result = store.add(CoverageappEntry(title: "Overflow"))
        XCTAssertFalse(result)
    }

    func testDeleteRemovesEntry() {
        let store = Store()
        _ = store.add(CoverageappEntry(title: "ToDelete"))
    ...
    func testUpdateChangesTitle() {
        let store = Store()
        _ = store.add(CoverageappEntry(title: "Original"))
    ...
    func testDeleteAtOffsets() {
        let store = Store()
        let before = store.entries.count
        _ = store.add(CoverageappEntry(title: "OffsetTest"))
